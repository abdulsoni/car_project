...
class UserController extends Controller
{
  ...
  public function VerifyBank(Request $request){
    $url = 'https://api.addressy.com/BankAccountValidation/Batch/Validate/v1.00/csv.ws?Key=EA42-GR59-AB79-RP19&AccountNumbers='.$request->accountNumber.'&SortCodes='.$request->sortCode;
    $ch = curl_init();
    ...
    $resp = array(
      "OriginalAccountNumber"=>$response[17],
      "OriginalSortCode"=>$response[18],
      "IsCorrect"=>$response[19],
      "IsDirectDebitCapable"=>$response[20],
      "StatusInformation"=>$response[21],
      "CorrectedSortCode"=>$response[22],
      "CorrectedAccountNumber"=>$response[23],
      "IBAN"=>$response[24],
      "Bank"=>$response[25],
      "BankBIC"=>$response[26],
      "Branch"=>$response[27],
      "BranchBIC"=>$response[28],
      "ContactAddressLine1"=>$response[29],
      "ContactAddressLine2"=>$response[30],
      "ContactPostTown"=>$response[31],
      "ContactPostcode"=>$response[32],
      "ContactPhone"=>$response[33]
    );
  	return $resp;
  }
  ...
  public function SaveApplication(Request $request){
    ...
    $lastOrder = SourceFilter::max('setOrder');
    // $lastOrder =  $Order->setOrder;
    // return $Order;

    function checkFilters($sourceFilters,$filters,$sendToAutoConvert,$currTimeStamp,$autoConvertApiKey,$actualDate){
      ...
      return array($filters,$autoConvertApiKey,$sendToAutoConvert,$sourceFilters->id);

    }
      if($sourceFilters->source==$utmSource){
          for($i=1;$i<=$lastOrder;$i++){
              $sourceFilterss = SourceFilter::where('setOrder',$i)->where('status','on')->first();
              if(!is_null($sourceFilterss)){
                $retFilters = checkFilters($sourceFilterss,$filters,$sendToAutoConvert,$currTimeStamp,$autoConvertApiKey,$actualDate);
                $filters = $retFilters[0];
                $autoConvertApiKey = $retFilters[1];
                $sendToAutoConvert = $retFilters[2];
                $sourceId = $retFilters[3];
                if($filters==true){
                  break;
                }
              }
    ...
    if($userTable->save())
    {
      ...
      if(is_null($sourceFilters)){
        ...
        if($filters==false){
          ...
          return "1#".$leadId."#".$sourceId."#no#".$sendToAutoConvert."#".$autoConvertApiKey;
        }
      }
      ...
      // return "1#".$leadId;
        ...
        if(is_null($sourceFilters)){
          ...
          if($filters==false){
            ...
            return "3#".$leadId."#".$sourceId."#no#".$sendToAutoConvert."#".$autoConvertApiKey;
          }
        }
    ...
    /* Add Activity Log Start */
      ...
      if(is_null($sourceFilters)){
        ...
        if($filters==false){
          ...
          return "2#".$leadId."#".$sourceId."#no#".$sendToAutoConvert."#".$autoConvertApiKey;
        }
      }
 ...
 public function sendEmails(){
  ...
  public function ViewMemberArea(){
  ...
  public function UpdateReference(Request $request){
    $user = User::where('leadId',$request->leadID)->first();
    $user['auto_reference'] = $request->reference;
    $user->save();
    if($user->save())
    {
      return 1;
